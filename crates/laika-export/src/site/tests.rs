...
fn opts(out: &Path) -> BuildOpts {
    BuildOpts {
        ...
        sizes: None,
        fonts_dir: Some(PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/../../assets/fonts"))),
...
fn synth(p: &SitePhoto, _max: u32) -> Result<image::RgbaImage, String> {
    let (w, h) = if p.photo_id % 2 == 1 { (2400, 1600) } else { (800, 1200) };
    Ok(image::RgbaImage::from_pixel(w, h, image::Rgba([(p.photo_id * 40) as u8, 90, 120, 255])))
}
...
fn derivative_edges_never_upscale() {
    assert_eq!(derivative_edges(3000, 2000, &[640, 1280, 2048]), vec![640, 1280, 2048]);
    assert_eq!(derivative_edges(800, 1200, &[640, 1280, 2048]), vec![640, 1200]);
...
fn build_tree_manifest_and_incremental_rebuilds() {
    ...
    for f in ["index.html", "manifest.json", "assets/gallery.js", "assets/fonts/IBMPlexSans-Regular.ttf"] {
        assert!(out.join(f).is_file(), "{f}");
    }
    let m = Manifest::read(&out).unwrap();
    assert_eq!(m.photos.len(), 6);
    assert_eq!(m.photos[0].files.iter().map(|f| f.width).collect::<Vec<_>>(), vec![640, 1280, 2048]);
    // Portrait 800×1200: long edges 640 and 1200 → widths 426 and 800.
    let portrait = &m.photos.iter().find(|p| p.photo_id == 2).unwrap().files;
    assert_eq!(portrait.iter().map(|f| (f.width, f.height)).collect::<Vec<_>>(), vec![(426, 640), (800, 1200)]);
    for p in &m.photos {
        for f in &p.files {
    ...
    assert_eq!(html.matches("data-full=").count(), 5, "photo 4 doesn't open full size");
    ...
    assert_eq!(preview_diff(&g, &photos, &out).summary(), "6 unchanged, page changed");
    let r = build(&g, &photos, &opts(&out), &render, |_| {}, &cancel).unwrap();
    assert_eq!((r.rendered, r.reused, renders.load(Ordering::Relaxed)), (0, 6, 0));
    assert!(std::fs::read_to_string(out.join("index.html")).unwrap().contains("New caption"));
    ...
    assert_eq!((d.reencoded.clone(), d.unchanged, d.page_changed), (vec![5], 5, false));
    let old_file = m.photos.iter().find(|p| p.photo_id == 5).unwrap().files[0].file.clone();
    ...
    assert!(build(&g, &edited, &opts(&out), &render, |_| {}, &cancelled).unwrap_err().contains("cancelled"));
    assert_eq!(std::fs::read_to_string(out.join("index.html")).unwrap(), before);
    ...
    let flaky = |p: &SitePhoto, m: u32| {
        if p.photo_id == 3 { Err("original is offline".to_string()) } else { synth(p, m) }
    };
    let mut changed = edited.clone();
    changed[2].edit_key = "e2".into();
    let r = build(&g, &changed, &opts(&out), flaky, |_| {}, &cancel).unwrap();
    assert_eq!(r.failures, vec![("DSCF0003.jpg".to_string(), "original is offline".to_string())]);
...
fn empty_gallery_refuses_to_build() {
    ...
    let e = build(&g, &[], &opts(&dir.join("x")), synth, |_| {}, &AtomicBool::new(false)).unwrap_err();
...
fn fake_files(n: usize) -> Vec<Vec<Derivative>> {
    (0..n)
        .map(|i| {
            vec![
                Derivative { width: 640, height: 427, file: format!("img/k{i}-640.jpg"), bytes: 1 },
                Derivative { width: 2048, height: 1365, file: format!("img/k{i}-2048.jpg"), bytes: 1 },
            ]
        })
        .collect()
}
...
fn page_placements_match_the_engine_for_every_template() {
    ...
    for t in TEMPLATES {
        ...
        let page: Vec<PagePhoto> = (0..14).map(|i| PagePhoto { index: i, files: &files[i] }).collect();
        let html = render_index(&g, &page, "Laika test");
        assert_balanced(&html);
        let desk = g.layout_at(Breakpoint::Desktop);
        assert_eq!(tile_vars(&html, "dc"), desk.iter().map(|r| r.1.col + 1).collect::<Vec<_>>(), "{}", t.id);
        assert_eq!(tile_vars(&html, "dr"), desk.iter().map(|r| r.1.row + 1).collect::<Vec<_>>(), "{}", t.id);
        assert_eq!(tile_vars(&html, "dsx"), desk.iter().map(|r| r.2 as u16).collect::<Vec<_>>(), "{}", t.id);
...
fn golden_index_html() {
    ...
    let page: Vec<PagePhoto> = (0..5).map(|i| PagePhoto { index: i, files: &files[i] }).collect();
    let html = render_index(&g, &page, "Laika test");
    let golden = PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/golden/gallery-index.html"));
...
fn demo_site_from_folder() {
    ...
    let render = |p: &SitePhoto, _m: u32| {
        ...
        image::open(f).map(|i| i.to_rgba8()).map_err(|e| e.to_string())
    };
    let r = build(&g, &photos, &opts(&out), &render, |_| {}, &AtomicBool::new(false)).unwrap();
