//! CSS-style letter-spacing. GPUI's `TextStyle` has no tracking field, so this shapes the line once
//! (kerning intact) and paints each glyph shifted right by `tracking` for every glyph before it.
//! Like CSS, the spacing also follows the last glyph, so measured widths match the mock.
            ...
            move |bounds, _, window, _| {
                ...
                for run in line.runs.iter() {
                    for g in &run.glyphs {
                        ...
                        window.paint_glyph(origin, run.font_id, g.id, font_size, color).ok();
                        i += 1;
                    }
                }
            },
